...
class Establishment extends CI_Controller {
	...
	private function _info_rating() {
		...
		//echo '<pre>' . print_r($this->_data['establishment'], true); exit;
	...
	public function city() {		
		...
		if (count($this->_data['establishments']) > 0) {
			foreach ($this->_data['establishments'] as $establishment) {
				...
				$establishment->categories = $this->EstablishmentModel->getCategoryForEstablishment($establishment->establishmentID);
			}
		}
	...
	public function migrate()
	{
		...
		exit;
		$this->EstablishmentModel->migrate();
	}
	...
	{		
		if ($this->_data['logged']) {
			...
			if ($this->_data['reviewCount'] >= MIN_REVIEW_COUNT_FOR_ESTABLISHMENT) {
				...
				/*if (count($this->_data['establishment']->categories) == 1 && 
					in_array('brewery', $this->_data['establishment']->categories)
				) {
					$this->_data['possible'] = false;
				}*/
    ...
    private function _processGoogleCoords(&$establishment) {
    	if (empty($establishment->latitude) || empty($establishment->longitude)) {
			...
			$this->google->determineLatitudeAndLongitudeViaGoogleCurl();
	...
	public function googleMaps() {
		...
		$this->load->model('EstablishmentModel', '', true);
		$establishment = $this->EstablishmentModel->getEstablishmentByID($establishmentID);
        ...
        $this->_data['establishment'] = $establishment;
        $this->_data['seo'] = $this->seo->getDynamicSEO($establishment, 'google maps');
