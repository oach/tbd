...
class Brewery extends CI_Controller
{
	public function __construct()
	{
        ...
        $this->_data['js'] = $this->jscript->loadRequired();
        $this->_data['search_option_value'] = 'establishment';
	}
	...
	{		
		...
		if ($this->_data['establishmentID']) {
			...
			if (!empty($this->_data['establishment'])) {
				$this->_data['establishment']->categories = $this->EstablishmentModel->getCategoryForEstablishment($this->_data['establishmentID']);
				...
				if (!empty($this->_data['beers'])) {
					...
					$price_package = $this->BreweriesModel->getPricePackageByEstablishmentId($this->_data['establishmentID']);
					$this->_data['price_per_ounce'] = $this->establishments->calculateCostPerOunce($price_package);
		...
		if ($this->_data['logged']) {
			...
			if ($this->_data['reviewCount'] >= MIN_REVIEW_COUNT_FOR_ESTABLISHMENT) {								
				...
				if (!$this->form_validation->run('addEstablishment')) {
					//echo '<pre>' . print_r(validation_errors(), true); exit;
					$this->_data['categories'] = $this->BreweriesModel->getAllCategoriesForDropDown();
					//echo '<pre>' . print_r($this->_data['categories'], true); exit;
					$this->_data['states'] = $this->StateModel->getAllForDropDown();
					$this->_data['css'] = ['bootstrap-multiselect.css'];
					array_push($this->_data['js'], 'establishment_add.js', 'bootstrap-multiselect.js');
				}
				else {//echo 'here'; exit;
					...
					$this->EstablishmentModel->addCategoryForEstablishment($this->input->post('category'), $id);
	...
	public function categoriesExists()
	{
		...
		//echo '1<pre>' . print_r($categories, true); exit;
		if (is_array($categories) && count($categories) > 0) {
			$this->load->model('BreweriesModel', '', true);
			foreach ($categories as $category) {
				$boolean = $this->BreweriesModel->getCategoryCheck($category);
				if (!$boolean) {
					$this->form_validation->set_message('categoryExists', 'The %s you have chosen doesn\'t exists.  Please choose another.');
					break;
				}
			...
			$this->form_validation->set_message('categoryExists', 'You must select a category.');
		}
		/*$this->load->model('BreweriesModel', '', true);
		echo '<pre>' . print_r($categoryID, true); exit;
		$rs = $this->BreweriesModel->getCategoriesCheck($categoryID);
		$boolean = count($rs) > 0 ? true : false;
		if (!$boolean) {
			$this->form_validation->set_message('categoryExists', 'The %s you have chosen doesn\'t exists.  Please choose another.');
		}
	...
	public function specialRequired($categoryID) {
		echo '<pre>' . print_r($categoryID, true); exit;
		/*array_pop($category);
		//echo '<pre>' . print_r($category, true); exit;
		$boolean = count($category > 0) ? true : false;
		if (!$boolean) {
			$this->form_validation->set_message('specialRequired', 'This a required field.');
		}
		return $boolean;*/
	}
