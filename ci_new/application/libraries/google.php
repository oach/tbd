...
class Google
{
	...
	public function determineLatitudeAndLongitudeViaGoogle() {
        ...
        //echo '<pre>' . print_r($post_array, true); exit;
    ...
    public function determineLatitudeAndLongitudeViaGoogleCurl()
    {
    	$ch = curl_init();
        $post_array = array(
            'e_id' => $this->getId(),
        ...
        curl_setopt($ch, CURLOPT_URL, 'https://maps.googleapis.com/maps/api/geocode/json?' . http_build_query($post_array));
        ...
        if (strtolower($json->status) == 'ok') {
        	$this->setLatitude($json->results[0]->geometry->location->lat);
        	$this->setLongitude($json->results[0]->geometry->location->lng);
        }      
