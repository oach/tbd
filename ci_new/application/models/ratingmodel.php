...
class RatingModel extends CI_Model
{
		...
		$this->db
			->select(
				...
				(ratings.aroma * (' . PERCENT_AROMA . ' / 100)) + 
					...
					(ratings.mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100)) +
					(ratings.overall * (' . PERCENT_OVERALL . ' / 100)) AS rating,
				ratings.comments,
				ratings.haveAnother,
				ratings.price,
				ratings.shortrating,
	...
	public function getAllPagination($limit)
	{
		...
		$query = '
			SELECT
				...
				, r.color
				, (aroma * (' . PERCENT_AROMA . ' / 100)) + (taste * (' . PERCENT_TASTE . ' / 100)) + (look * (' . PERCENT_LOOK . ' / 100)) + (mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100)) + (overall * (' . PERCENT_OVERALL . ' / 100)) AS rating
				, r.comments
				, r.haveAnother
				, r.price
				, r.shortrating
		...
		$this->db
			->select(
				...
				DATE_FORMAT(ratings.dateAdded, "%W, %M %D, %Y at %T") AS dateAdded,
				ratings.color,
				(ratings.aroma * (' . PERCENT_AROMA . ' / 100)) + (ratings.taste * (' . PERCENT_TASTE . ' / 100)) + (ratings.look * (' . PERCENT_LOOK . ' / 100)) + (ratings.mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100)) + (ratings.overall * (' . PERCENT_OVERALL . ' / 100)) AS rating,
				ratings.comments,
				ratings.haveAnother,
				ratings.price,
				ratings.shortrating,
    ...
    public function getBeerByStylesRatedByUserIDForUserProfile($user_id, $style_id) {
    	$this->db
    		->select(
                ...
                ratings.dateTasted,
                ratings.comments,
                ROUND((ratings.aroma * (' . PERCENT_AROMA . ' / 100)) + (ratings.taste * (' . PERCENT_TASTE . ' / 100)) + (ratings.look * (' . PERCENT_LOOK . ' / 100)) + (ratings.mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100) + (ratings.overall * (' . PERCENT_OVERALL . ' / 100)), 1) AS rating,
                beers.id AS beer_id,
                beers.beerName,
                beers.alcoholContent,
                beers.picture,
    ...
    public function getBeerRatingByUserIDStatistics($id) {
        ...
        $query = '
            SELECT
                ...
                AVG((ratings.aroma * (' . PERCENT_AROMA . ' / 100)) + (ratings.taste * (' . PERCENT_TASTE . ' / 100)) + (ratings.look * (' . PERCENT_LOOK . ' / 100)) + (ratings.mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100)) + (ratings.overall * (' . PERCENT_OVERALL . ' / 100))) AS rated_beer_average,
                COUNT(DISTINCT styles.id) AS rated_styles,
                MAX((ratings.aroma * (' . PERCENT_AROMA . ' / 100)) + (ratings.taste * (' . PERCENT_TASTE . ' / 100)) + (ratings.look * (' . PERCENT_LOOK . ' / 100)) + (ratings.mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100)) + (ratings.overall * (' . PERCENT_OVERALL . ' / 100))) AS rated_beer_max,
                MIN((ratings.aroma * (' . PERCENT_AROMA . ' / 100)) + (ratings.taste * (' . PERCENT_TASTE . ' / 100)) + (ratings.look * (' . PERCENT_LOOK . ' / 100)) + (ratings.mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100)) + (ratings.overall * (' . PERCENT_OVERALL . ' / 100))) AS rated_beer_min
            FROM users
    ...
    public function getEstablishmentRatingByUserIDStatistics($id, $return = 'array')
    {
    	...
    	$this->db
    		->select(
    			'establishment_has_categories.categoryID,
    			COUNT(users.id) AS rated_establishments,
                AVG((rating_establishment.drink * (' . PERCENT_DRINK . ' / 100)) + (rating_establishment.service * (' . PERCENT_SERVICE . ' / 100)) + (rating_establishment.atmosphere * (' . PERCENT_ATMOSPHERE . ' / 100)) + (rating_establishment.pricing * (' . PERCENT_PRICING . ' / 100)) + (rating_establishment.accessibility * (' . PERCENT_ACCESSIBILITY . ' / 100))) AS rated_establishment_average,
                establishment_categories.name,
                MAX((rating_establishment.drink * (' . PERCENT_DRINK . ' / 100)) + (rating_establishment.service * (' . PERCENT_SERVICE . ' / 100)) + (rating_establishment.atmosphere * (' . PERCENT_ATMOSPHERE . ' / 100)) + (rating_establishment.pricing * (' . PERCENT_PRICING . ' / 100)) + (rating_establishment.accessibility * (' . PERCENT_ACCESSIBILITY . ' / 100))) AS rated_establishment_max,
    		...
    		->join('rating_establishment', 'rating_establishment.userID = users.id', 'left outer')
    		->join('establishment', 'establishment.id = rating_establishment.establishmentID', 'inner')
    		->join('establishment_has_categories', 'establishment_has_categories.establishmentID = establishment.id', 'inner')
    		->join('establishment_categories', 'establishment_categories.id = establishment_has_categories.categoryID', 'inner')
    		->where('users.id', $id)
    		->where('users.active', '1')
    		->where('users.banned', '0')
    		->group_by('establishment_categories.name')
	...
	public function getEstablishmentRatingsByUserIDAndCategoryID($user_id, $category_id) {
		$this->db
			...
			->join('establishment_has_categories', 'establishment_has_categories.establishmentID = rating_establishment.establishmentID', 'inner')
			->where('rating_establishment.userID', $user_id)
			->where('establishment_has_categories.categoryID', $category_id)
	...
	public function updateRatingByID($data) {
		...
		$query = '
			UPDATE ratings SET
				...
				, mouthfeel = ' . mysqli_real_escape_string($this->db->conn_id, htmlentities($data['mouthfeel'])) . '
				, overall = ' . mysqli_real_escape_string($this->db->conn_id, htmlentities($data['overall'])) . '
				, comments = "' . mysqli_real_escape_string($this->db->conn_id, htmlentities($data['comments'])) . '"	
				, haveAnother = "' . mysqli_real_escape_string($this->db->conn_id, $data['haveAnother']) . '"			
				, price = ' . mysqli_real_escape_string($this->db->conn_id, $data['price']) . '
			WHERE
	...
	public function createRating($data) {
		...
		$query = '
			INSERT INTO ratings (
				...
				, ' . mysqli_real_escape_string($this->db->conn_id, $data['mouthfeel']) . '
				, ' . mysqli_real_escape_string($this->db->conn_id, $data['overall']) . '
				, "' . mysqli_real_escape_string($this->db->conn_id, htmlentities($data['comments'])) . '"
				, "' . mysqli_real_escape_string($this->db->conn_id, $data['haveAnother']) . '"
				, ' . mysqli_real_escape_string($this->db->conn_id, $data['price']) . '
				, "1"
