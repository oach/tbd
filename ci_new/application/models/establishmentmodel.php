...
class EstablishmentModel extends CI_Model
{
	...
	public function getEstablishmentsByCategory($stateID) {		
		$query = '
			SELECT
				...
				COUNT(establishment_has_categories.categoryID) AS totalPerCategory
			FROM establishment_categories
			LEFT OUTER
				JOIN establishment_has_categories
					ON establishment_has_categories.categoryID = establishment_categories.id
			INNER 
				JOIN establishment 
					ON establishment.id = establishment_has_categories.establishmentID
			...
			GROUP BY
				...
				establishment_categories.name
			ORDER BY
				IF(COUNT(establishment_has_categories.categoryID) > 0, 1, 0) DESC,
	...
	{
		if (isset($config['lat']) && isset($config['long'])) {
			$data = [
				'latitude' => $config['lat'],
	...
	public function determineDistance(StdClass $establishment) {
        ...
        $this->db
        	...
        	//->limit(LIMIT_RADIUS_DISTANCE_SEARCH);
    ...
    public function addCategoryForEstablishment($categories, $id)
	{
		if (is_array($categories) && count($categories) > 0) {
			$array = [];
			foreach ($categories as $category) {
				$array[] = ['categoryID' => $category, 'establishmentID' => $id];
			}
			$this->db->insert_batch('establishment_has_categories', $array);
		}
	}

	public function getCategoryForEstablishment($id)
	{
		$this->db
			->select('establishment_categories.id, establishment_categories.name')
			->from('establishment_has_categories')
			->join('establishment_categories', 'establishment_categories.id = establishment_has_categories.categoryID', 'inner')
			->where('establishment_has_categories.establishmentID', $id);
	...
	{
		$this->db
			->select('establishment.id, establishment.categoryID')
			->from('establishment')
			->where('establishment.categoryID IS NOT NULL', null, false);

		$rs = $this->db->get();
		$foreach = '';
        if ($rs->num_rows() > 0) {
        	foreach ($rs->result() as $row) {
        		$foreach .= '(' . $row->id . ', ' . $row->categoryID . '),';
        	}
        }

        $query = '
			INSERT INTO establishment_has_categories (
				establishmentID,
				categoryID
			) VALUES
		';
		...
		$query .= rtrim($foreach, ',');
		//echo '<pre>' . print_r($query, true); exit;
		$this->db->query($query);
	}
}
?>