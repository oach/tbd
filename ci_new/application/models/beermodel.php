...
class BeerModel extends CI_Model
{
    ...
    public function getTrendByBeerID($id) {
        $query = '
            SELECT
                AVG((aroma * (' . PERCENT_AROMA . ' / 100)) + (taste * (' . PERCENT_TASTE . ' / 100)) + (look * (' . PERCENT_LOOK . ' / 100)) + (mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100)) + (overall * (' . PERCENT_OVERALL . ' / 100))) AS trendrating
            FROM (
                SELECT
                    r.aroma
                    , r.taste
	...
	public function getBeerRating($id)
	{
		...
		$this->db
			->select(
				...
				AVG((ratings.aroma * (' . PERCENT_AROMA . ' / 100)) + (ratings.taste * (' . PERCENT_TASTE . ' / 100)) + (ratings.look * (' . PERCENT_LOOK . ' / 100)) + (ratings.mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100)) + (ratings.overall * (' . PERCENT_OVERALL . ' / 100))) AS averagerating',
				false
			)
			->from('beers')
	...
	public function getBestWorstStyles($type)
	{
		...
		$this->db
			->select(
				'styles.id,
				styles.style,
				ROUND(AVG((ratings.aroma * (' . PERCENT_AROMA . ' / 100)) + (ratings.taste * (' . PERCENT_TASTE . ' / 100)) + (ratings.look * (' . PERCENT_LOOK . ' / 100)) + (ratings.mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100)) + (ratings.overall * (' . PERCENT_OVERALL . ' / 100))), 1) AS avgRating,
				COUNT(ratings.id) AS totalRatings,
	...
	{
		$this->db
			->select('AVG((ratings.aroma * (' . PERCENT_AROMA . ' / 100)) + (ratings.taste * (' . PERCENT_TASTE . ' / 100)) + (ratings.look * (' . PERCENT_LOOK . ' / 100)) + (ratings.mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100)) + (ratings.overall * (' . PERCENT_OVERALL . ' / 100))) AS avergeRating', false)
			->from('beers')
			->join('establishment', 'establishment.id = beers.establishmentID', 'inner')
			->join('ratings', 'ratings.beerID = beers.id', 'inner')
			->join('users',  'users.id = ratings.userID', 'inner')
		...
		$rs = $this->db->get();
		//echo '<pre>' . print_r($this->db->last_query(), true); exit;
        if ($rs->num_rows() > 0) {
        	return $rs->row();
		}
		return [];
	...
	public function getNumBeersAndAverageByUserID($userID)
	{
		$this->db
			->select(
				...
				ROUND(AVG((ratings.aroma * (' . PERCENT_AROMA . ' / 100)) + (ratings.taste * (' . PERCENT_TASTE . ' / 100)) + (ratings.look * (' . PERCENT_LOOK . ' / 100)) + (ratings.mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100)) + (ratings.overall * (' . PERCENT_OVERALL . ' / 100))), 1) AS rating',
				false
			)
			->from('beers')
			->join('establishment', 'establishment.id = beers.establishmentID', 'inner')
    ...
    public function autoCompleteSearch($term) {
    	$this->db
    		->select(
    			'beers.id,
    			CONCAT(beers.beerName, "<br>", establishment.name) AS name,
    			beers.slug,
    			beers.picture',
    			false
    		)
