...
class BreweriesModel extends CI_Model {
	...
	public function getAllRatingsForBreweryByID($id) {
		$this->db
			->select(
				...
				styles.style,
				AVG((ratings.aroma * (' . PERCENT_AROMA . ' / 100)) + (ratings.taste * (' . PERCENT_TASTE . ' / 100)) + (ratings.look * (' . PERCENT_LOOK . ' / 100)) + (ratings.mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100)) + (ratings.overall * (' . PERCENT_OVERALL . ' / 100))) AS averagereview,
	...
	public function getAllActiveRatingsForBreweryByID($id) {
		$this->db
			->select(
				...
				styles.id AS styleID,
				styles.style,
				ROUND(AVG((ratings.aroma * (' . PERCENT_AROMA . ' / 100)) + (ratings.taste * (' . PERCENT_TASTE . ' / 100)) + (ratings.look * (' . PERCENT_LOOK . ' / 100)) + (ratings.mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100)) + (ratings.overall * (' . PERCENT_OVERALL . ' / 100))), 1) AS averagereview,
				COUNT(DISTINCT ratings.id) AS reviews,
	...
	public function getTotalEachBeer($id) {
		$this->db
			->select(
				...
				SUM(CASE WHEN ratings.rating IS NOT NULL THEN ratings.rating ELSE ((ratings.aroma * (' . PERCENT_AROMA . ' / 100)) + (ratings.taste * (' . PERCENT_TASTE . ' / 100)) + (ratings.look * (' . PERCENT_LOOK . ' / 100)) + (ratings.mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100)) + (ratings.overall * (' . PERCENT_OVERALL . ' / 100))) END) AS totalPoints',
				false
			)
			->from('beers')
	...
	public function getHighestRatedBreweries() {
		$this->db
			->select(
				...
				AVG(CASE WHEN ratings.rating IS NOT NULL THEN ratings.rating ELSE ((ratings.aroma * (' . PERCENT_AROMA . ' / 100)) + (ratings.taste * (' . PERCENT_TASTE . ' / 100)) + (ratings.look * (' . PERCENT_LOOK . ' / 100)) + (ratings.mouthfeel * (' . PERCENT_MOUTHFEEL . ' / 100)) + (ratings.overall * (' . PERCENT_OVERALL . ' / 100))) END) as avgRating'
			)
			->from('beers')
			->join('establishment', 'establishment.id = beers.establishmentID', 'inner')
			->join('ratings', 'ratings.beerID = beers.id', 'inner')
	...
	public function getPricePackageByEstablishmentId($id)
	{
		$this->db
			->select(
				'ratings.price,
